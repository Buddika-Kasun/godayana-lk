...
export function CourseForm({
  ...
  const [selectedMigrationPaths, setSelectedMigrationPaths] = useState<
    string[]
  >(initialData?.migrationPaths || []);
  const [currentMigrationPath, setCurrentMigrationPath] = useState("");
  ...
  const [formData, setFormData] = useState({
    ...
    migrationPath: initialData?.migrationPaths || [],
    requirementLevel: initialData?.requirementLevel || "",
  });
      ...
      try {
        ...
        if (apiResponse.success && apiResponse.data) {
          ...
          setFormData({
            ...
            migrationPath: course.migrationPaths || [],
            requirementLevel: course.requirementLevel || "",
          });
          ...
          setCourseImageFileKey(course.courseImageFileKey || "");
           if (course.migrationPaths && Array.isArray(course.migrationPaths)) {
             setSelectedMigrationPaths(course.migrationPaths);
           } else if (course.migrationPaths) {
             // Backward compatibility: if it's a single string, convert to array
             setSelectedMigrationPaths([course.migrationPaths]);
           } else {
             setSelectedMigrationPaths([]);
           }
        } else {
  ...
  // Migration Path handlers
  const handleAddMigrationPath = () => {
    if (
      currentMigrationPath.trim() &&
      !selectedMigrationPaths.includes(currentMigrationPath.trim())
    ) {
      setSelectedMigrationPaths([
        ...selectedMigrationPaths,
        currentMigrationPath.trim(),
      ]);
      setCurrentMigrationPath("");
    }
  };

  const handleRemoveMigrationPath = (path: string) => {
    setSelectedMigrationPaths(selectedMigrationPaths.filter((p) => p !== path));
  };

  const handleMigrationPathKeyDown = (
    e: React.KeyboardEvent<HTMLInputElement>,
  ) => {
    if (e.key === "Enter") {
      e.preventDefault();
      handleAddMigrationPath();
    }
  };

  const handleMigrationPathSelect = (value: string) => {
    if (value && !selectedMigrationPaths.includes(value)) {
      setSelectedMigrationPaths([...selectedMigrationPaths, value]);
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.migrationPath.length > 0) {
      toast.error("Migration paths are required");
      return false;
    }
    if (!formData.requirementLevel) {
      toast.error("Requirement level is required");
      return false;
    }
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const requestData: CourseRequest = {
        ...
        migrationPaths:
          selectedMigrationPaths.length > 0
            ? selectedMigrationPaths
            : undefined,
  ...
  return (
    <form onSubmit={(e) => handleSubmit(e, "publish")} className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        {/* Migration Path - Multi-select with chips */}
        <div>
          <Label
            htmlFor="migrationPath"
            className="text-sm font-semibold text-primary"
          >
            Migration Paths / Destinations{" "}
          ...
          {/* Select dropdown for adding paths */}
          <Select
            onValueChange={(value) => {
              if (value && !selectedMigrationPaths.includes(value)) {
                setSelectedMigrationPaths([...selectedMigrationPaths, value]);
          ...
          >
            <SelectTrigger className="mt-1.5 w-full">
              <SelectValue placeholder="Add migration destination..." />
            </SelectTrigger>
            <SelectContent>
              {migrationPaths.map((path) => (
                <SelectItem
                  key={path.value}
                  value={path.value}
                  disabled={selectedMigrationPaths.includes(path.value)}
                >
                  <div className="flex items-center justify-between w-full">
                    <span>{path.label}</span>
                    {selectedMigrationPaths.includes(path.value) && (
          ...
          {selectedMigrationPaths.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              {selectedMigrationPaths.map((path) => {
                const pathLabel =
                  migrationPaths.find((p) => p.value === path)?.label || path;
                return (
                  <Badge
                    key={path}
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => handleRemoveMigrationPath(path)}
          ...
          {selectedMigrationPaths.length === 0 && (
            <p className="text-xs text-muted-foreground mt-2">
              No migration destinations selected
            </p>
          )}
        </div>
        ...
        <div>
          <Label
            ...
            className="text-sm font-semibold text-primary"
          >
            Requirements Level <span className="text-red-500">*</span>
          </Label>
          <Select
