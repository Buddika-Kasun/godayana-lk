...
const navItems: NavItem[] = [
  ...
  { name: "Payment Management", href: "/admin/payments", icon: Coins },
  { name: "Content Management", href: "/admin/content", icon: Settings },
  ...
  useEffect(() => {
    ...
    if (!isLoading && !hasRedirected.current) {
      ...
      if (!isAuthenticated) {
        ...
        hasRedirected.current = true;
        setTimeout(() => {
          setRedirectMessage("Access denied. Admin only area.");
          setShowRedirectMessage(true);
        }, 0);
        setTimeout(() => {
          router.push("/");
