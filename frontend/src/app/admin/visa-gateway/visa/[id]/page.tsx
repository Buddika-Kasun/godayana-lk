...
import adminVisaGatewayEndpoints, {
  ...
  VisaConsultationCountryResponse,
...
export default function VisaConsultationsPage() {
  ...
  // Filters
  const [visaTypeFilter, setVisaTypeFilter] = useState<string>("STUDENT");
  ...
  const fetchConsultations = useCallback(async () => {
    ...
    try {
      ...
      const response =
        await adminVisaGatewayEndpoints.visa.getConsultationByCountry(params);
  ...
  }, [country, currentPage, itemsPerPage, visaTypeFilter, statusFilter]);
  ...
  const fetchCounts = useCallback(async () => {
    try {
      ...
      const response =
        await adminVisaGatewayEndpoints.visa.countAdminConsultationsByCountry(
          params,
        );
      const apiResponse = response.data;
  ...
  useEffect(() => {
    ...
    fetchConsultations();
    fetchCounts();
  }, [visaTypeFilter, statusFilter, fetchConsultations, fetchCounts]);
  ...
  const handleStatusChange = async (
    ...
    try {
      const response =
        await adminVisaGatewayEndpoints.visa.updateConsultationStatus(
          consultationId,
          newStatus,
        );
  ...
  // Get counts
  const studentCount = counts.student || 0;
  const workCount = counts.work || 0;
  const visitCount = counts.visit || 0;
        ...
        <CardContent className="flex-1 flex flex-col px-4">
          ...
          <div className="w-full mb-4">
            <div className="bg-primary/10 p-1 rounded-lg w-full lg:w-fit flex items-center gap-1 flex-wrap">
              ...
              >
                All ({studentCount + workCount + visitCount})
              </button> */}
              <button
                onClick={() => {
          ...
          <div className="w-full flex flex-col md:flex-row justify-between mb-4 border-b pb-3">
            <div className="bg-primary/10 p-1 rounded-lg w-full lg:w-fit flex items-center gap-1 flex-wrap">
              ...
              >
                All Status (
                {pendingCount + inReviewCount + completedCount + cancelledCount}
                )
              </button> */}
              <button
                onClick={() => {
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12">
                <p className="text-muted-foreground">
                  {visaTypeFilter
                    ? `No ${getVisaTypeLabel(visaTypeFilter).toLowerCase()} consultations found`
                    : statusFilter
                      ? `No ${getStatusConfig(statusFilter).label.toLowerCase()} consultations found`
              ...
              filteredConsultations.map((consultation) => {
                ...
                const visaTypeLabel = getVisaTypeLabel(consultation.type);
                          ...
                          >
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t">
                              ...
                              <div className="space-y-1">
                                ...
                                <Badge
                                  variant={
                                    consultation.hasPassport
                                      ? "default"
                                      : "destructive"
                                  }
                                  className="w-fit"
                                >
                                  {consultation.hasPassport ? "Yes" : "No"}
                                </Badge>
                              </div>
                              <div className="space-y-1">
                                <p className="text-xs font-semibold text-muted-foreground">
                                ...
                                <Badge
                                  variant={
                                    consultation.visaRejection
                                      ? "destructive"
                                      : "default"
                                  }
                                  className="w-fit"
                                >
                                  {consultation.visaRejection ? "Yes" : "No"}
                                </Badge>
                              </div>
                              <div className="space-y-1">
                                <p className="text-xs font-semibold text-muted-foreground">
                                ...
                                <p className="text-sm font-medium">
                                  {formatDateDisplay(consultation.travelDate)}
                                </p>
                              </div>
