// src/app/company/courses/leads/[id]/page.tsx
"use client";
...
import { CourseApplicationCountsResponse, CourseApplicationParams } from "@/lib/api/endpoints/seeker/seekerCourseEndpoints";
import companyCourseEndpoints, { CompanyCourseApplicationResponse } from "@/lib/api/endpoints/company/companyCourseEndpoints";
...
export default function CourseApplicationsPage() {
  ...
  const [applications, setApplications] = useState<
    CompanyCourseApplicationResponse[]
  >([]);
  ...
  const fetchCounts = useCallback(async () => {
    try {
      const appCountRes =
        await companyCourseEndpoints.enrollments.getCompanyCourseCount(
          courseId,
        );
  ...
  const fetchApplications = useCallback(
    async (filter: typeof activeFilter, page: number = 1) => {
      ...
      try {
        ...
        const response =
          await companyCourseEndpoints.enrollments.getEnrollmentsByCourse(
            courseId,
            params,
          );
  ...
  const handleStatusChange = async (
    ...
    setIsUpdating(true);
    try {
      const response = await companyCourseEndpoints.enrollments.updateEnrollmentStatus(
        applicationId,
        newStatus,
      );
  ...
  const getCountForFilter = (filter: string) => {
    switch (filter) {
      ...
      case "enrolled":
        return applicationCounts.enrolled;
      case "rejected":
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-4 pb-4 border-b">
        <Button
          ...
          variant="ghost"
          size="sm"
          onClick={() => router.push(`/company/courses`)}
                  ...
                  >
                    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex flex-col md:flex-row gap-2">
                          {isPending && (
                            <>
                              <Button
                                ...
                                onClick={() =>
                                  handleStatusChange(application.id, "ENROLLED")
                                }
                                disabled={isUpdating}
                              >
                                <UserCheck size={14} />
                          ...
                          {isEnrolled && (
                            <Button
                              ...
                              className="flex-1 py-1 gap-2 cursor-pointer"
                              onClick={() =>
                                handleStatusChange(application.id, "CANCELLED")
                              }
                              disabled={isUpdating}
                            >
                              <UserX size={14} />
                        ...
                        <div className="">
                          <Link
                            href={`/company/courses/leads/seeker/${application.seekerId}?application=${application.id}`}
                          >
                            <Button
                              variant="outline"
                              size="sm"
                              className="w-full py-1 gap-1 cursor-pointer"
                            >
                              <Eye size={14} />
                              View Profile
                            </Button>
